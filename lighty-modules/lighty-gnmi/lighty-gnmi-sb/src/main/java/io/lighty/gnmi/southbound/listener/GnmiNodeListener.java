...
public class GnmiNodeListener implements DataTreeChangeListener<Node> {
    ...
    public void onDataTreeChanged(@NonNull List<DataTreeModification<Node>> changes) {
        ...
        for (final DataTreeModification<Node> change : changes) {
            ...
            final NodeId nodeId = IdentifierUtils.nodeIdOfPathArgument(rootNode.step());
            switch (rootNode.modificationType()) {
                ...
                case SUBTREE_MODIFIED:
                    if (nodeParamsUpdated(rootNode)) {
                        ...
                        connectNode(rootNode.dataAfter());
                    }
                    break;
                case DELETE:
                    ...
                    break;
                default:
                    LOG.warn("Unsupported tree modification received, {}", rootNode.modificationType());
                    break;
            }
    ...
    private void disconnectNode(final NodeId nodeId) {
        ...
        writeTransaction.delete(LogicalDatastoreType.OPERATIONAL, IdentifierUtils.gnmiNodeIID(nodeId).toIdentifier());
        try {
    ...
    private boolean nodeParamsUpdated(final DataObjectModification<Node> rootNode) {
        final Node nodeBefore = rootNode.dataBefore();
        final Node nodeAfter = rootNode.dataAfter();
    ...
    private void writeConnectionFailureReasonToDatastore(NodeId nodeId, String message)
        ...
        tx.merge(LogicalDatastoreType.OPERATIONAL, IdentifierUtils.gnmiNodeIID(nodeId).toIdentifier(), operationalNode);
